...
void tabulate(void)
{
    ...
    for( int voter_num = 0; voter_num < voter_count; voter_num++)
    {

        for (int prior = 0; prior < 3; prior++){

            int candidate_num = preferences[voter_num][prior];
...
int find_min(void)
{
    ...
    for(int j = 0; j < candidate_count; j++)
    {
        ...
        loser_votes = candidates[j].votes;
    }
    for(int i = 0; i < candidate_count; i++)
    {
        if(candidates[i].eliminated == 0)
        {
            if(candidates[i].votes < loser_votes)
            {
