...
void sepia(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    for(a = 0; a < height; a = a + 1)
    {
        ...
        for (b = 0; b < width; b = b + 1)
        {
            sepiared= round(0.393*image[a][b].rgbtRed + 0.769*image[a][b].rgbtGreen + 0.189*image[a][b].rgbtBlue);
            sepiagreen = round(0.349*image[a][b].rgbtRed + 0.686*image[a][b].rgbtGreen + 0.168*image[a][b].rgbtBlue);
            sepiablue = round(0.272*image[a][b].rgbtRed + 0.534*image[a][b].rgbtGreen + 0.131*image[a][b].rgbtBlue);
