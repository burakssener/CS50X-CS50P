        ...
        if not symbol:
            ...
            if not request.form.get("stock_num"):
                ...
                if (user_data["cash"] >= symbol["price"] * stock_num):
                    ...
                    user_data = db.execute("SELECT users_balance.stock_num AS Shares, users_balance.stock_name AS Name, (users_balance.stock_num * ) AS Total FROM users JOIN users_balance ON users_balance.user_id = users.id WHERE id = ?", session['user_id'])[0]
