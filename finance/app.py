    ...
    if request.method == "GET":
        ...
        if not symbol:
            ...
            if not request.form.get("stock_num"):
                ...
                user_data = db.execute("SELECT id, cash, stock_num, stock_name FROM users JOIN users_balance ON users_balance.user_id = users.id WHERE id = ?", session['user_id'])[0]
                if (user_data["cash"] >= symbol["price"] * stock_num ):
                    ...
                    user_data = db.execute("SELECT * FROM users JOIN users_balance ON users_balance.user_id = users.id WHERE users.id = ?", session['user_id'])
